 ...
 * 
 */
public class VodListBackCtrBot extends PopupWindow {
	...
	public VodListBackCtrBot(Context context, ListVideoView videoView,
			Handler handler) {
	...
	// public VodListBackCtrBot(Context context, BackVideoView videoView,
	// Handler handler) {
	// this.mContext = context;
	// this.mVideoView = videoView;
	// this.mHandler = handler;
	...
	public void showAtLocation(View parent, int gravity, int x, int y) {
		...
		mHandler.postDelayed(hide, 3 * 1000);
		setProgress();
	}
	...
	private Runnable hide = new Runnable() {
		...
		@Override
		public void run() {
			if (VodListBackCtrBot.this.isShowing()) {
				dismiss();
			}
