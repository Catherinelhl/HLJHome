...
import com.hlj.net.GetShouCangMetroDataResponse.MetroData.ActionExtra;
import com.hlj.net.GetShouCangMetroDataResponse.MetroData.ActionExtra.Item;
import com.hlj.utils.JsonTools;
import com.hlj.utils.Logger;
import com.live.video.net.BaseResponse;

public class GetShouCangMetroDataResponse extends BaseResponse {
	...
	public void paseRespones(String js) {
		...
		try {
			...
			for (int i = 0; i < array.length(); i++) {
				...
				if ("openUrl".equals(info.mode)) {
					...
					if (JSONObject.NULL == extraJson.get("item")
							|| "".equals(extraJson.get("item"))
							|| null == extraJson.get("item")) {
			...
			Logger.log("GetShouCangMetroDataResponse:" + e.toString());
		}
