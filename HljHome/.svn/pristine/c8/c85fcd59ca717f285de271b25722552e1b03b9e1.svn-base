...
public class Util {
    ...
    public static boolean hasCompatibleCPU()
    {
        ...
        if("x86".equals(ANDROID_ABI) && !hasX86) {
            errorMsg = "x86 build on non-x86 device";
            isCompatible = false;
            return false;
        } else if(hasX86 && "armeabi".contains(ANDROID_ABI)) {
            errorMsg = "ARM build on x86 device";
            isCompatible = false;
            return false;
        }

        if("mips".equals(ANDROID_ABI) && !hasMips) {
            errorMsg = "MIPS build on non-MIPS device";
            isCompatible = false;
            return false;
        } else if(hasMips && "armeabi".contains(ANDROID_ABI)) {
            errorMsg = "ARM build on MIPS device";
            isCompatible = false;
            return false;
        }

        if("armeabi-v7a".equals(ANDROID_ABI) && !hasArmV7) {
            errorMsg = "ARMv7 build on non-ARMv7 device";
            isCompatible = false;
            return false;
        }
        if("armeabi".equals(ANDROID_ABI)) {
        ...
        if("armeabi".equals(ANDROID_ABI) || "armeabi-v7a".equals(ANDROID_ABI)) {
            if(!hasNeon) {
