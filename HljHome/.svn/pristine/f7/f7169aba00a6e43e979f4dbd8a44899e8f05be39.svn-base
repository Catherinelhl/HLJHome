...
public class MediaPlayerVideo extends Activity implements
	...
	public void onCreate(Bundle icicle) {
		...
		if (null != info.standVideo.urls && info.standVideo.urls.size() > 0) {
			setDataSource(info.standVideo.urls);

			title.setText(info.title + "(" + info.items[0] + ")");
		} else if (null != info.highVideo.urls
				&& info.highVideo.urls.size() > 0) {
			setDataSource(info.highVideo.urls);

			title.setText(info.title + "(" + info.items[1] + ")");
		} else if (null != info.superVideo.urls
				&& info.superVideo.urls.size() > 0) {
			setDataSource(info.superVideo.urls);

			title.setText(info.title + "(" + info.items[2] + ")");
		}
	...
	public void createDialog() {
		Logger.log("createDialog 执行====================");
		 AlertDialog.Builder builder=new AlertDialog.Builder(this).setTitle("切换清晰度")
				.setItems(info.items, new DialogInterface.OnClickListener() {

					@Override
					public void onClick(DialogInterface dialog, int which) {

						long currentPostion = mMediaPlayer.getCurrentPosition();
						...
						case 0:
							...
							setDataSource(info.standVideo.urls);
							// 标清
							playVideo(STREAM_VIDEO, currentPostion);
							break;
						case 1:
							title.setText(info.title + "(" + info.items[1]
									+ ")");
